...
class AgentEmployeeUserNotSelectedException(HTTPException):
    def __init__(        self,
        status_code=HTTP_401_UNAUTHORIZED,
        detail=error_strings.AGENT_EMPLOYEE_USER_NOT_SELECTED_ERROR,
        headers=None,
    ):
        super().__init__(
            status_code,
...
class InactiveDeviceException(HTTPException):
    def __init__(
        self, status_code=HTTP_403_FORBIDDEN, detail=error_strings.INACTIVE_DEVICE_ERROR
    ):
        super().__init__(
            status_code=status_code,
            detail=detail,
