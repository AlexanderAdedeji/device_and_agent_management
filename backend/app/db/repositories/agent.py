...
from backend.app.schemas.agent import AgentCreateForm, AgentCreate
...
class AgentRepository(Base[Agent]):
    def get_by_email(self, db: Session, *, email: str) -> Agent:
        agent = db.query(Agent).filter(Agent.email == email).first()
        return agent

    def get_name(self, db:Session,*, id=int)->Agent:
